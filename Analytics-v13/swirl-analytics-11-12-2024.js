...
async function initializeVideoDatassv13(videoId, videoUrl, playlist) {
    ...
    const videoData = existingVideoData || {
        ...
        total_views: 0,
        duration: allDataofSwirls.swirls[playlist] ? allDataofSwirls.swirls[playlist]?.find((el) => {
            return el.server_url == videoUrl;
        })?.video_len : allDataofSwirls.swirls['video']?.find((el) => {
            return el.server_url == videoUrl;
        })?.video_len,
        video_title: allDataofSwirls.swirls[playlist] ? allDataofSwirls.swirls[playlist]?.find((el) => {
            return el.server_url == videoUrl;
        })?.video_title : allDataofSwirls?.swirls['video']?.find((el) => {
            return el.server_url == videoUrl;
        })?.video_title,
        drop_of_point: [],
        skip_points: [],
...
async function impressionCountssv13(brand_id, playlistCode, videoidsArray) {
    ...
    // let userGeoData = await getUserGeossv13();
    // let ipData = await fetchIPInfossv13(userGeoData.ipAddress);
    ...
    let payload2 = {
        ...
        // fingerprint: newUUID,
        // browserName: userGeoData.browserName,
        // osName: userGeoData.osName,
        // deviceType: userGeoData.deviceType,
        // userAgent: userGeoData.userAgent,
        // ipAddress: userGeoData.ipAddress,
        // browserVersion: userGeoData.browserVersion,
        // city: ipData.city,
...
// async function getUserGeossv13() {
//     // Initialize an object to store user information
//     const userData = {};
...
//     } catch (error) {
//         userData.ipAddress = 'Unable to fetch IP';
//     }
...
//     userData.browserName = browserName;
//     userData.browserVersion = browserVersion;
//     userData.osName = osName;
//     userData.deviceType = deviceType;

//     // userData.brandId = ssv13_brandCustomizations.brand_id;
//     // userData.playlistId = ssv13_storePlaylist.split(',')[mNo];

//     // localStorage.setItem(`ssv13_userGeo_${ssv13_brandCustomizations.brand_id}_${mNo}`, Math.floor(Date.now() / 1000))
...
// const fetchIPInfossv13 = async (ip) => {
//     if (localStorage.getItem(`ssv13_ip_${ip}`)) {
//         // console.log("FROM LS");
//         return JSON.parse(localStorage.getItem(`ssv13_ip_${ip}`));
//     } else {
//         // console.log("FROM API");
//         const token = window.atob('NGI4NzBkMDQ2MDZjMWI=');
//         try {
...
//             localStorage.setItem(`ssv13_ip_${ip}`, JSON.stringify({ city, region, country }));
//             return { city, region, country };
//         } catch (error) {
